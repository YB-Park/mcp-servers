...
function encodeHash(hash: Buffer): string {
...
function parseStoredKey(value: unknown): StoredApiKey {
  ...
  return {
    ...
    createdAt: key.createdAt,
    ...(optionalString('subject') ? { subject: optionalString('subject') } : {}),
    ...(optionalString('expiresAt') ? { expiresAt: optionalString('expiresAt') } : {}),
    ...(optionalString('revokedAt') ? { revokedAt: optionalString('revokedAt') } : {}),
    ...(optionalString('replacedBy') ? { replacedBy: optionalString('replacedBy') } : {}),
  };
}
...
export class FileApiKeyStore {
  ...
  private issue(document: StoreDocument, input: CreateApiKeyInput, now: Date): IssuedApiKey {
    ...
    const stored: StoredApiKey = {
      ...
      tokenHash: encodeHash(hashToken(token)),
      scopes,
